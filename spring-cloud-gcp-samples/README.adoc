...
- link:spring-cloud-gcp-oracle-kubernetes-sample[Oracle Database on Kubernetes Sample]:
Provides comprehensive Kubernetes manifests and documentation for deploying Oracle Database in Kubernetes environments, including Azure Kubernetes Service (AKS) and Google Kubernetes Engine (GKE). Includes deployment configurations, Spring Boot integration examples, and best practices for running Oracle in containers.
